...
export function calculateMatchScore(
  ...
  if (sourceDog.gender === targetDog.gender) {
    return {
      dogId: '',
      score: 0,
      reasons: ['Same gender - not compatible'],
      distance,
    }
  }
  ...
  if (sourceDog.temperament && targetDog.temperament) {
    ...
    if (commonTraits.length > 0) {
      score += 10
      reasons.push(
        `Compatible temperament (${commonTraits.length} common traits)`,
      )
    }
  }

  const normalisedScore = Math.max(
    0,
    Math.min(100, Math.round((score / MAX_SCORE) * 100)),
  )

  return { dogId: '', score: normalisedScore, reasons, distance }
}
